...
---------------------
!pip install module_name # jupyter内使用
!pip install --upgrade package_name
!pip install model_name==0.1.20
!pip uninstall model_name

conda install package_name # jupyter内使用
conda update package_name

numpy.__version__

pip list # 安装的所有包
# python的安装路径'F:\Install\python3.7\Lib'下，点击site-packages
...
-------------------------------------------
s = pd.Series([4, 7, -5, 3], index=) # pd.Series([4, 7, -5, 3], index=['d', 'b', 'a', 'c'])
...
s[s > 2] # s[np.abs(s) > 3]
s.unique()  # 去重
s[s.isin(['a', 'b', 'c'])] # s.isin(['a', 'b', 'c']) / True False # 筛选包含['a', 'b', 'c']的行
